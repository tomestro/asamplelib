...
lint: ## check style with flake8
	poetry run flake8 asamplelib tests

format: ## run autoformat with isort and black
	poetry run isort
	poetry run black asamplelib tests
...
coverage: ## check code coverage quickly with the default Python
	poetry run coverage run --source asamplelib -m pytest
	poetry run coverage report -m
	poetry run coverage html
	$(BROWSER) htmlcov/index.html

docs: ## generate Sphinx HTML documentation, including API docs
	rm -f docs/asamplelib.rst
	rm -f docs/modules.rst
	poetry run sphinx-apidoc -o docs/ asamplelib
	$(MAKE) -C docs clean
	$(MAKE) -C docs html
